...
if HTTPTOOLS_DEFAULT_DOWNLOAD_TIMEOUT == 0:
...
def read_body_and_headers(
    url, post=None, headers=None, follow_redirects=False, timeout=None
):
...
def get_user_agent(quoted=False):
    ...
    if quoted:
        resp = urlparse.quote(default_headers["User-Agent"])
...
def get_cookie(url, name, follow_redirects=False):
    ...
    for cookie in cj:
        if (
            cookie.name == name or (name_like and name_like in cookie.name)
        ) and domain in cookie.domain:
            return cookie.value
    return False
...
def get_url_headers(url, forced=False, dom=False):
    ...
    if ("|" in url or "cf_clearance" not in domain_cookies) and not forced:
        return url
    ...
    headers["Cookie"] = "; ".join(
        ["%s=%s" % (c.name, c.value) for c in list(domain_cookies.values())]
    )

    return (
        url
        + "|"
        + "&".join(["%s=%s" % (h, urlparse.quote(headers[h])) for h in headers])
    )
...
def load_CF_list(domain="", **opt):
    ...
    try:
        CF_LIST_PATH = os.path.join(
            config.get_runtime_path(), "resources", "CF_Domains.txt"
        )
...
def channel_proxy_list(url, forced_proxy=None):
    try:
        proxy_channel_bloqued_str = base64.b64decode(
            config.get_setting("proxy_channel_bloqued")
        ).decode("utf-8")
    ...
    if proxy_channel_bloqued.get(domain, ""):
        ...
        if forced_proxy:
            return True
        if "OFF" not in proxy_channel_bloqued[domain]:
            return True
...
def check_proxy(url, **opt):
    ...
    proxy_data["proxy__test"] = opt.get("proxy__test", "") or opt.get(
        "canonical", {}
    ).get("proxy__test", "")
    proxy_data["proxy"] = opt.get("proxy", True) or opt.get("canonical", {}).get(
        "proxy", True
    )
    proxy_data["proxy_web"] = opt.get("proxy_web", False) or opt.get(
        "canonical", {}
    ).get("proxy_web", False)
    proxy_data["proxy_addr_forced"] = opt.get("proxy_addr_forced", None) or opt.get(
        "canonical", {}
    ).get("proxy_addr_forced", None)
    proxy_data["forced_proxy"] = opt.get("forced_proxy", None) or opt.get(
        "canonical", {}
    ).get("forced_proxy", None)
    proxy_data["force_proxy_get"] = opt.get("force_proxy_get", False) or opt.get(
        "canonical", {}
    ).get("force_proxy_get", False)
    proxy_data["forced_proxy_opt"] = opt.get("forced_proxy_opt", "")
    proxy_data["forced_proxy_ifnot_assistant"] = opt.get(
    ...
    if (
        ...
        and (
            proxy_data["forced_proxy"]
            or proxy_data["proxy_addr_forced"]
            or channel_proxy_list(url, forced_proxy=proxy_data["forced_proxy"])
        )
        and str(proxy_data["forced_proxy_opt"]) != "reset"
        and not config.get_setting("external_vpn", default=False)
    ):
        if PY3:
        ...
        proxy_data, opt = proxytools.get_proxy_addr(
            url, forced_proxy_m=proxy_data["forced_proxy"], proxy_data=proxy_data, **opt
        )
        url = proxy_data["url"]
...
def proxy_stat(url, proxy_data, **opt):
    ...
    if proxy_data.get("proxy__test", "") == "retry":
        ...
        try:
            proxy_white_list_str = base64.b64decode(
                config.get_setting("proxy_white_list")
            ).decode("utf-8")
    ...
    if "Proxy Direct" in proxy_data["stat"]:
        return "ProxyDirect|%s||%s" % (proxy_data.get("log", ""), retry)
    if "Proxy CF" in proxy_data["stat"]:
        return "ProxyCF|%s||%s" % (proxy_data.get("log", ""), retry)
    if "Proxy Web" in proxy_data["stat"]:
        return "ProxyWeb|%s|%s|%s" % (
            proxy_data.get("web_name", ""),
            obtain_domain(proxy_data.get("url", ""), scheme=True),
            retry,
        )
...
def proxy_post_processing(url, proxy_data, response, **opt):
    ...
    try:
        if response["code"] in NOT_FOUND_CODES:
            ...
            response["code"] not in SUCCESS_CODES + REDIRECTION_CODES
            and opt.get("forced_proxy_opt", "") == "ProxyJSON"
        ...
        if ", Proxy Web" in proxy_data.get("stat", ""):
            ...
            if not (
                ...
                response = proxytools.restore_after_proxy_web(
                    response, proxy_data, **opt
                )
                data = response["data"][:500]
            ...
            if data.startswith("ERROR") or response["code"] in REDIRECTION_CODES:
                if response["code"] in SUCCESS_CODES + [666]:
                    ...
                    if not opt.get("post_cookie", False):
                        ...
                        data_domain = downloadpage(
                            url_domain,
                            alfa_s=True,
                            ignore_response_code=True,
                            headers=opt.get("headers", {}),
                            CF_test=opt.get("CF_test", False),
                            post_cookie=True,
                            forced_proxy=forced_proxy_temp,
                ...
                elif response["code"] in REDIRECTION_CODES and response["headers"].get(
                    "location", ""
                ):
                    response["sucess"] = True
                if data.startswith("ERROR") or (
                    response["code"] in REDIRECTION_CODES and not response["sucess"]
                ):
        ...
        if (
            ...
            if ", Proxy Direct" in proxy_data.get(
                "stat", ""
            ) or ", Proxy CF" in proxy_data.get("stat", ""):
                if ", Proxy Direct" in proxy_data.get("stat", ""):
                    proxy_init = "ProxyDirect"
                if ", Proxy CF" in proxy_data.get("stat", ""):
                    proxy_init = "ProxyCF"
                ...
                if opt.get("CF_proxy_alt", True) and (channel_proxy_list(
                    ...
                    proxytools.get_proxy_list_method(
                        proxy_init="ProxyWeb", error_skip=proxy_data["web_name"], **opt
                    )
                    url = opt["url_save"]
                    opt["post"] = opt["post_save"]
...
def blocking_error(url, req, proxy_data, **opt):
    if not opt.get("canonical_check", True):
        return False
    if not opt.get("error_check", True):
        return False
    code = str(req.status_code) or ""
    data = ""
    if req.content:
        data = req.content[:5000]
    ...
    if (
        "104" in code
        or "10054" in code
        or ("404" in code and "Object not found" in data and ".torrent" not in url)
        or (
            "502" in code
            and ("Por causas ajenas" in data or "Contenido bloqueado" in data)
        )
        or ("sslcertverificationerror") in code.lower()
        or ("certificate verify failed") in code.lower()
        or (
            opt.get("check_blocked_IP", False)
            and "Please wait while we try to verify your browser..." in data
        )
    ):
        resp = True
        ...
        if (
            ...
            logger.debug(
                'ERROR 99: IP bloqueada por la Web "%s".  Recupenrando...' % req.url
            )
            domain = obtain_domain(req.url, sub=True)
            domains = [domain]
            if domain in base64.b64decode(
                config.get_setting("proxy_channel_bloqued")
            ).decode("utf-8"):
                try:
                    pCFa = base64.b64decode(config.get_setting("proxy_CF_addr")).decode(
                        "utf-8"
                    )
                    pCFl_str = base64.b64decode(
                        config.get_setting("proxy_CF_list")
                    ).decode("utf-8")
                ...
                if not pCFl:
                    if canonical.get("host_alt", []):
                        ...
                        proxytools.add_domain_retried(
                            ...
                            proxy__type=opt.get(
                                "forced_proxy",
                                opt.get("forced_proxy_retry", "") or "ProxyCF",
                            ),
                            delete=True,
                        )
                    resp = False
                    ...
                    config.set_setting(
                        "proxy_CF_addr",
                        base64.b64encode(
                            str(pCFl[0] if len(pCFl) > 0 else "").encode("utf-8")
                        ).decode("utf-8"),
                    )
                    config.set_setting(
                        "proxy_CF_list",
                        base64.b64encode(str(pCFl).encode("utf-8")).decode("utf-8"),
                    )
            else:
                if not PY3:
                    from . import proxytools
                ...
                proxytools.add_domain_retried(
                    domains,
                    proxy__type=opt.get(
                        "forced_proxy", opt.get("forced_proxy_retry", "") or "ProxyCF"
                    ),
                    delete=True,
                )
        ...
        if code:
            ...
            try:
                if not CACHING_DOMAINS:
                    logger.error(
                        "Error: %s, Url: %s, Datos: %s" % (code, url, data[:500])
                    )
            except Exception:
                if not CACHING_DOMAINS:
...
def canonical_quick_check(url, **opt):
    ...
    alfa_s = True
    if not canonical.get("alfa_s", alfa_s):
        logger.info(url, force=True)

    url_host = scrapertools.find_single_match(url, patron_host).rstrip("/") + "/"
    if (
        url_host
        and url_host in canonical.get("host_black_list", [])
        and canonical.get("host_alt", [])
    ):
    ...
    if "btdig" in canonical.get("host", "") and canonical.get("channel", ""):
        ...
        canonical["host"] = ""
        config.set_setting(
            "current_host", canonical["host"], channel=canonical["channel"]
        )
...
def canonical_check(url, response, req, **opt):
    ...
    alfa_s = True
    if not opt.get("canonical", {}).get("alfa_s", alfa_s):
        logger.info(url, force=True)

    canonical_host = ""
    data = ""
    if response["data"]:
        data = response["data"][:250000]
    ...
    if (
        canonical.get("host", "")
        and canonical.get("channel", "")
        and (
            "hideproxy" in canonical["host"]
            or "webproxy" in canonical["host"]
            or "hidester" in canonical["host"]
            or "hide" in canonical["host"]
            or "croxyproxy" in canonical["host"]
            or "__cpo=" in canonical["host"]
            or "btdig" in canonical["host"]
        )
    ):
    ...
    for pattern in patterns:
        ...
        if len(canonical_host) < 8:
            canonical_host = ""
        if canonical_host:
            canonical_host = canonical_host.replace("\\", "")
            if (
                canonical_host
                and not canonical_host.startswith("http")
                and canonical_host.startswith("//")
            ):
                canonical_host = "https:%s" % canonical_host
            if canonical_host and (
                "hideproxy" in canonical_host
                or "webproxy" in canonical_host
                or "hidester" in canonical_host
                or "hide" in canonical_host
                or "croxyproxy" in canonical_host
                or "__cpo=" in canonical_host
                or "btdig" in canonical_host
            ):
                canonical_host = ""
                continue
    ...
    if (
        canonical.get("host", "")
        and canonical.get("host_black_list", [])
        and canonical.get("host_alt", [])
    ...
    if (
        response["sucess"]
        and response["canonical"]
        and canonical.get("channel", "")
        and (canonical.get("host", "") or canonical.get("host_alt", []))
    ):
        if response["canonical"] != canonical.get("host", "") and response[
            "canonical"
            ...
            if response["canonical"] in url:
                ...
                if (
                    canonical.get("host", "")
                    and canonical["host"] not in canonical.get("host_black_list", [])
                ...
                for url_alt in host_list:
                    ...
                    page = downloadpage(
                        ...
                        url_save=opt.get("url_save", ""),
                        session_verify=opt.get("session_verify", False),
                        forced_proxy_ifnot_assistant=opt.get(
                            "forced_proxy_ifnot_assistant", ""
                        ),
                    )
                    if page.sucess:
                    ...
                    if (
                        canonical_new_alt
                        and canonical.get("host", "")
                        and canonical["host"] in canonical.get("host_black_list", [])
                    ):
            ...
            if canonical_new and canonical_new != canonical.get("host", ""):
                logger.info(
                    'Canal: %s: Host "%s" cambiado a CANONICAL: %s - %s'
                    % (
                        canonical["channel"].capitalize(),
                        canonical.get("host", ""),
                        response["canonical"],
                        canonical_new if canonical_new != response["canonical"] else "",
                    ),
                    force=True,
                )
...
def retry_alt(url, req, response_call, proxy_data, **opt):
    ...
    if (
        opt.get("url_save")
        and opt["url_save"] in canonical["host_alt"]
        and str(req.status_code) != "400"
    ):
    ...
    if str(req.status_code) == "400" and canonical["host_alt"]:
        if not PY3:
            ...
            from . import proxytools_py3 as proxytools
        proxytools.add_domain_retried(
            obtain_domain(canonical["host_alt"][0], sub=True), proxy__type="ProxyWeb"
        )
    ...
    logger.error(
        'ERROR 98: Web "%s" caída, reintentando con %s'
        % (opt.get("url_save", host_a), canonical["host_alt"])
    )
    config.set_setting(
        "current_host", "", channel=canonical["channel"]
    )  # Reseteamos el dominio

    try:
        channel = __import__(
            "channels.%s" % canonical["channel"],
            None,
            None,
            ["channels.%s" % canonical["channel"]],
        )
    ...
    for host_b in canonical["host_alt"]:
        ...
        if (
            proxy_data.get("stat", "")
            and "Proxy Web" not in proxy_data.get("stat", "")
            and not TEST_ON_AIR
        ):
        ...
        if "proxy_web" in opt:
            del opt["proxy_web"]
        if "forced_proxy" in opt:
            del opt["forced_proxy"]
...
def reset_canonical(canonical_new, url, response, **opt):
    ...
    if not isinstance(response, dict):
        ...
        if not url.startswith("magnet"):
            response.url_new = url.replace(
                scrapertools.find_single_match(url, patron_host),
                canonical_new.rstrip("/"),
            )
    else:
        response["host"] = canonical_new
        if not url.startswith("magnet"):
...
def downloadpage(url, **opt):
    ...
    DEBUG = (
        config.get_setting("debug_report", default=False) if not TEST_ON_AIR else False
    )
    ...
    if VIDEOLIBRARY_UPDATE and "hide_infobox" not in opt:
        opt["hide_infobox"] = True
    ...
    if "retries_cloudflare" not in opt:
        opt["retries_cloudflare"] = opt.get("canonical", {}).get(
            "retries_cloudflare", 0
        )
    if "CF" not in opt and "CF_stat" in opt.get("canonical", {}):
        opt["CF"] = opt["CF_stat"] = opt["canonical"]["CF_stat"]
    if "cf_assistant" not in opt and "cf_assistant" in opt.get("canonical", {}):
        opt["cf_assistant"] = opt["canonical"]["cf_assistant"]
    if "session_verify" not in opt and "session_verify" in opt.get("canonical", {}):
        opt["session_verify"] = opt["canonical"]["session_verify"]
    if "session_verify_save" not in opt:
        opt["session_verify_save"] = (
            opt["session_verify"] if "session_verify" in opt else None
        )
    if "CF_if_assistant" not in opt and "CF_if_assistant" in opt.get("canonical", {}):
        opt["CF_if_assistant"] = opt["canonical"]["CF_if_assistant"]
    if "CF_if_NO_assistant" not in opt and "CF_if_NO_assistant" in opt.get(
        "canonical", {}
    ):
    ...
    if (opt.get("set_tls", "") or opt.get("set_tls", "") is None) and opt.get(
        ...
        ssl_version = opt["set_tls"]
    if "set_tls_min" not in opt and "set_tls_min" in opt.get("canonical", {}):
        opt["set_tls_min"] = opt["canonical"]["set_tls_min"]
    ...
    if "cf_assistant_get_source" not in opt and "cf_assistant_get_source" in opt.get(
        "canonical", {}
    ):
    ...
    # Preparando la url
    if not PY3:
        url = urlparse.quote(url.encode("utf-8"), safe="%/:=&?~#+!$,;'@()*[]")
    else:
    ...
    if opt.get("random_headers", False) or HTTPTOOLS_DEFAULT_RANDOM_HEADERS:
        ...
        platform = (
            __platform.replace("raspberry", "linux")
            .replace("osx", "darwin")
            .replace("xbox", "windows")
            .replace("tvos", "ios")
            .replace("atv2", "android")
            .replace("unknown", "windows")
        )
    ...
    while opt["proxy_retries_counter"] <= opt.get("proxy_retries", 1):
        ...
        if (
            not proxy_data.get("stat", "")
            and opt.get("CF_if_assistant", False)
            and opt.get("CF_if_NO_assistant", True)
        ):
            opt["CF"] = opt["cloudscraper_active"] = True
        if (
            "croxyproxy.com" in proxy_data.get("web_name", "")
            or "CF_stat" in opt
            and not opt["CF_stat"]
        ):
        ...
        if (
            (domain in CF_LIST or opt.get("CF", False))
            and opt.get("CF_test", True)
            and opt.get("cloudscraper_active", True)
        ):  # Está en la lista de CF o viene en la llamada
            from lib.cloudscraper import create_scraper

            session = create_scraper(
                user_url=url, user_opt=opt
        ...
        if (
            ...
            and ssl_version
            and (opt.get("set_tls", "") or SET_TLS)
        ):
            ssl_version = (
                ssl_version
                if ssl_version_min or not opt.get("set_tls_min", False)
                else None
            )
        ...
        if (
            opt.get("timeout", None) is None
            and HTTPTOOLS_DEFAULT_DOWNLOAD_TIMEOUT is not None
        ):
        ...
        if len(url) > 0:
            try:
                if (
                    opt.get("post", None) is not None
                    or opt.get("file", None) is not None
                    or opt.get("files", {})
                ):
                    ...
                    if opt.get("files", {}):
                        ...
                        if len(opt["file"]) < 256 and os.path.isfile(opt["file"]):
                            ...
                            files = {
                                "file": (opt.get("file_name", "Default"), opt["file"])
                            }
                            file_name_ = (
                ...
                if (
                    block
                    and opt.get("retry_alt", retry_alt_default)
                    ...
                    url_host = (
                        url_host + "/"
                        if url_host and not url_host.endswith("/")
                        else ""
                    )
                    if (
                        canonical.get("host_alt", [])
                        and canonical.get("host_black_list", [])
                        ...
                        if not opt.get("alfa_s", False):
                            logger.error(
                                "Error: %s in url: %s - Reintentando con %s"
                ...
                if (
                    not opt.get("ignore_response_code", False)
                    and not proxy_data.get("stat", "")
                    and not block
                ):
                    if (
                        opt.get("retry_alt", retry_alt_default)
                    ...
                    if opt.get("canonical", {}).get("channel", ""):
                        config.set_setting(
                            "current_host", "", channel=opt["canonical"]["channel"]
                        )
                        opt["canonical"]["host"] = ""
        ...
        if (
            ...
            opt["forced_proxy"] = (
                ...
                or forced_proxy_web
                if opt.get("forced_proxy_ifnot_assistant", "") == "ProxySSL"
                else opt.get("forced_proxy_ifnot_assistant", "") or forced_proxy_web
            )
        ...
        if (
            block
            and opt.get("check_blocked_IP", False)
            and opt.get("proxy__test", "") != "retry"
            and not proxy_data.get("stat", "")
            and opt.get("proxy_retries", 1)
        ):
            if not opt.get("alfa_s", False):
                logger.error(
                    "Error: %s en IP bloqueada: %s - Reintentando"
                    % (response_code, url)
                )
            opt["proxy_retries"] = 1 if PY3 and not TEST_ON_AIR else 0
            opt["proxy__test"] = "retry"
        ...
        if (
            (
                "timeout" in str(response_code).lower()
                or "Detected a Cloudflare version 2" in str(response_code)
            ...
            update_alfa_domain_web_list(
                url, "Timeout=%s" % str(opt["timeout"]), proxy_data, **opt
            )

            if not alfa_domain_web_list or not alfa_domain_web_list.get(
                proxy_data.get("web_name", ""), []
            ):
                ...
                if (
                    opt.get("forced_proxy_ifnot_assistant", "") in ["ProxySSL"]
                    or not channel_proxy_list(opt["url_save"])
                ) and filetools.exists(
                    filetools.join(
                        config.get_data_path(), "alfa-mobile-assistant.version"
                    )
                ):
                    opt["CF"] = opt["CF_save"] = opt["cloudscraper_active"] = opt[
                        "CF_test"
                    ...
                    opt["forced_proxy"] = None
                    if opt.get("canonical", {}):
                        opt["canonical"]["forced_proxy_ifnot_assistant"] = opt[
                            "forced_proxy_ifnot_assistant"
                        ]
                    ...
                    proxytools.add_domain_retried(
                        domain, proxy__type="", delete="forced"
                    )
                    if not CACHING_DOMAINS:
                        logger.debug(
                            "CF Assistant reTRY without SSL... for domain: %s" % domain
                        )
                ...
                proxytools.add_domain_retried(
                    domain, proxy__type="ProxyCF", delete="SSL"
                )
                return downloadpage(opt["url_save"], **opt)

            else:
                if isinstance(opt["timeout"], int):
                    opt["timeout"] += 3
                if isinstance(opt["timeout"], float):
                    opt["timeout"] += 3.0
                opt["post"] = opt["post_save"]
                return downloadpage(opt["url_save"], **opt)
        ...
        if (
            req.headers.get("Server", "").startswith("cloudflare")
            ...
            if (domain not in CF_LIST and opt["retries_cloudflare"] >= 0) or opt[
                "retries_cloudflare"
            ] > 0:
                if "__cpo=" not in url and domain not in CF_LIST:
                    CF_LIST += [domain]
                    save_CF_list(domain, **opt)
                update_alfa_domain_web_list(url, response_code, proxy_data, **opt)
                opt["proxy_retries"] = (
                    1
                    if PY3 and not TEST_ON_AIR
                    else 0
                    if opt["retries_cloudflare"] < 1
                    else 1
                )
                ...
                if "CF_save" not in opt:
                    opt["CF_save"] = opt["CF"] if "CF" in opt else None
                if opt["CF_save"] is None:
                    opt["CF"] = False if opt["retries_cloudflare"] > 0 else True
                if not CACHING_DOMAINS:
                    logger.debug(
                        "CF retry... for domain: %s, Retry: %s"
                        % (domain, opt["retries_cloudflare"])
                    )
                return downloadpage(opt["url_save"], **opt)
        ...
        if (
            opt.get("forced_proxy_ifnot_assistant", "")
            and (
                "Detected a Cloudflare version 2" in str(response_code)
                or response_code in CLOUDFLARE_CODES
            )
            ...
            if opt.get("cf_v2", False):
                ...
                info_dict, response = fill_fields_post(
                    url, info_dict, req, response, req_headers, inicio, **opt
                )
                show_infobox(info_dict, force=True)
            ...
            if not opt.get("CF_if_NO_assistant", True):
                opt["CF"] = opt["CF_save"] = opt["CF_stat"] = opt[
                    "session_verify"
                ] = opt["session_verify_save"] = opt["cloudscraper_active"] = False
            opt["forced_proxy"] = "ProxyCF"
            if opt["forced_proxy_ifnot_assistant"] in [
                "ProxySSL"
            ] and not proxy_data.get("stat", ""):
                opt["forced_proxy"] = "ProxyWeb:croxyproxy.com"
                opt["forced_proxy_ifnot_assistant"] = "ProxyCF"
            proxytools.add_domain_retried(
                domain, proxy__type=opt["forced_proxy"], delete="SSL"
            )
            return downloadpage(opt["url_save"], **opt)

        if (
            proxy_data.get("stat", "")
            and not proxy_data.get("web_name", "")
            and response["data"]
            and "<HTML><HEAD><TITLE>302 Moved</TITLE></HEAD>" in str(response["data"])
        ):
            if not PY3:
                from . import proxytools
            else:
        ...
        try:
            response["encoding"] = (
                str(req.encoding).lower()
                if req.encoding and req.encoding is not None
                else None
            )
        ...
        try:
            if (
                "bittorrent" not in req.headers.get("Content-Type", "")
        ...
        is_channel = scrapertools.find_single_match(
            str(is_channel), "<module '(channels).*?'"
        )
        if is_channel and isinstance(response_code, int):
            if not opt.get("ignore_response_code", False) and not proxy_data.get(
                "stat", ""
            ):
        ...
        if (
            not opt.get("alfa_s", False)
            and "api.themoviedb" not in url
            and "//127." not in url
        ):
        ...
        if opt.get("soup", False):
            try:
                ...
                response["soup"] = BeautifulSoup(
                    response["data"],
                    "html5lib",
                    from_encoding=opt.get("encoding", response["encoding"]),
                )
            ...
            if opt.get("check_blocked_IP", False) and not response["sucess"]:
                if opt.get("canonical", {}).get("host_alt", []):
                    if not PY3:
                        ...
                        from . import proxytools_py3 as proxytools
                    domains = []
                    for domain in [opt["canonical"]["host"]] + opt["canonical"][
                        "host_alt"
                    ]:
                        if not domain:
                            continue
                ...
                if opt.get("check_blocked_IP_save", {}):
                    ...
                    response_code = response["code"] = opt["check_blocked_IP_save"][
                        "code"
                    ]
                    response["sucess"] = True
            break
...
def show_infobox(info_dict, force=False):
    ...
    logger.info(
        "%s%s%s" % (box["r_up_corner"], box["fill"] * width, box["l_up_corner"]),
        force=force,
    )
    logger.info(
        "%s%s%s" % (box["center"], version.center(width), box["center"]), force=force
    )
    logger.info(
        "%s%s%s" % (box["r_center"], box["fill"] * width, box["l_center"]), force=force
    )
    ...
    for key, value in info_dict:
        ...
        if isinstance(text, list):
            for line in text:
                ...
                logger.info(
                    "%s%s%s" % (box["center"], line, box["center"]), force=force
                )
        else:
            logger.info("%s%s%s" % (box["center"], text, box["center"]), force=force)
        if count < len(info_dict):
            logger.info(
                "%s%s%s" % (box["r_center"], box["fill"] * width, box["l_center"]),
                force=force,
            )
        else:
            logger.info(
                "%s%s%s"
                % (box["r_dn_corner"], box["fill"] * width, box["l_dn_corner"]),
                force=force,
            )
    return
    ...
    try:
        ...
        info_dict.append(
            (
                "Dominio",
                "%s - Verify: %s"
                % (urlparse.urlparse(url)[1], opt.get("session_verify", False)),
            )
        )
        if CS_stat:
        ...
        if opt.get("set_tls_OK", False):
            info_dict.append(("SSL_TLS_version", ssl_version))
...
def fill_fields_post(url, info_dict, req, response, req_headers, inicio, **opt):
    try:
        ...
        if response["code"] in SUCCESS_CODES:
            info_dict.append(
                ("Success", "True - Error_check: %s" % opt.get("error_check", True))
            )
            response["sucess"] = True
        else:
            info_dict.append(
                ("Success", "False - Error_check: %s" % opt.get("error_check", True))
            )
            response["sucess"] = False
        ...
        info_dict.append(
            (
                ...
                0 if not response["data"] else len(response["data"]),
            )
        )
        ...
        url_new = None
        if (
            not response["proxy__"]
            and opt.get("url_save", "")
            and opt["url_save"] != response["url"]
        ):
            url_new = response["url"]
        if (
            not response["proxy__"]
            and opt.get("url_save", "")
            and response["url_new"]
            and opt["url_save"] != response["url_new"]
        ):
...
def update_alfa_domain_web_list(url, code, proxy_data, **opt):
    ...
    if ", Proxy Web" not in proxy_data.get("stat", ""):
        return
    if window:
        alfa_domain_web_list = (
            json.loads(window.getProperty("alfa_domain_web_list"))
            if window.getProperty("alfa_domain_web_list")
            else {}
        )
    if not alfa_domain_web_list:
        return
    ...
    for proxy_type, urls_list in list(alfa_domain_web_list.copy().items()):
        for x, (server, url_proxy) in enumerate(urls_list):
            ...
            if domain in url_proxy:
                ...
                window.setProperty(
                    "alfa_domain_web_list", json.dumps(alfa_domain_web_list)
                )

                if DEBUG:
                    logger.debug(
                        'ELIMINADO "%s" de %s: RAZÓN: %s'
                        % (url_proxy, alfa_domain_web_list, code)
                    )
