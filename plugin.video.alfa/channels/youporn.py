...
finds = {'find': dict([('find', [{'tag': ['div'], 'class': ['container']}]),
                       ('find_all', [{'tag': ['article'], 'class': ['video-box']}])]),
         'categories':  dict([('find', [{'tag': ['div'], 'class': ['popularPornstars-row', 'container', 'channelListWrapper']}]),
                              ('find_all', [{'tag': ['a', 'div'], 'class': ['categoryBox', 'card-link', 'performer-link']}])]),
                       # {'find_all': [{'tag': ['div'], 'class': ['porn-star-list']}]},
         'search': {}, 
         'get_quality': {}, 
         'get_quality_rgx': '', 
         'next_page': dict([('find', [{'tag': ['nav'],  'id': ['pagination']}]),
         ...
         # 'profile_labels': {'section_cantidad': dict([('find', [{'tag': ['span', 'div'], 'class': ['video-count', 'videoCount']}]),
                                                      # ('get_text', [{'strip': True}])])}, 
         'controls': {'url_base64': False, 'cnt_tot': 36, 'reverse': False, 'profile': 'default'}, 
         'timeout': timeout}
