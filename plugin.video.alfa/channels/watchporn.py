...
canonical = {
             'channel': 'watchporn', 
             'host': config.get_setting("current_host", 'watchporn', default=''), 
             'host_alt': ["https://watchporn.to/"], 
             'host_black_list': [], 
...
def mainlist(item):
    ...
    itemlist.append(Item(channel=item.channel, title="Canal" , action="section", url=host + "categories/?sort_by=total_videos&from=1", extra="Canal"))
